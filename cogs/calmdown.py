...
"""
    DISCORD_CALMDOWN_ROLE - Die Rollen-ID der "Cooldown"-Rolle.
...
class Calmdown(commands.Cog):
    ...
    async def unsilence(self, user_id, guild_id, inform_user=False):
        ...
        try:
            ...
            if inform_user:
                await utils.send_dm(user, f"Die Cooldown-Rolle wurde nun wieder entfernt.")
    ...
    @help(
        brief="Weist einem User die Cooldown-Rolle zu.",
    ...
    async def cmd_calmdown(self, ctx, user: disnake.Member, duration):
        ...
        if not utils.is_valid_time(duration):
            ...
            self.save()
            await ctx.channel.send(f"{ctx.author.mention} hat an {user.mention} die **Cooldown-Rolle** vergeben.")
